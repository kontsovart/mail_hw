...
class MyTargetClient:
    ...
    def add_cookie(self, mc, ssdc, mrcu, sdcs, z_token):
        self.mc_token = mc
    ...
    def add_segment_post(self, segment_name):
        headers = {
            ...
            'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) '
                          'Chrome/80.0.3987.162 Safari/537.36',
            'X-CSRFToken': self.session.cookies['csrf_token'],
        ...
        data = {"name": segment_name,
                ...
                "logicType": "or"}
        response = self.session.request("POST", urljoin(self.base_url, location), headers=headers, json=json.dumps(data))
        return response

    def get_cufuds(self):
        location = 'https://target.my.com/api/v2/campaigns_count.json'
        return self.session.request('GET', location)

    def get_sdfsa(self):
        location = 'https://target.my.com/api/v2/coverage/segment.json?id=837409'
        return self.session.request('GET', location)
